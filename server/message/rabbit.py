...
class BasicPikaClient:
    ...
    def declare_queue(
        self, queue_name, exclusive: bool = False, max_priority: int = None
    ):
...
def create_consumer(queue):
    worker = MyConsumer()
    worker.declare_queue(queue_name=queue)
    worker.declare_exchange(exchange_name="myexchange")
    worker.bind_queue(
        exchange_name="myexchange", queue_name=queue, routing_key="randomkey"
    )
    worker.consume_messages(queue=queue, callback=worker.consume)


if __name__ == "__main__":
    create_consumer("mike")